...
This is a demo of traffic simulation experiments using [UXsim](https://github.com/toruseo/UXsim), driven by LLM (Large Language Model).

## About This LLM Experiment

This entire project — including the traffic simulation design, data generation pipeline, machine learning model implementation, result analysis, and report writing — was produced by an LLM coding agent (GitHub Copilot) under minimal human guidance.

### How it worked

- **Human role**: The human provided only high-level direction through short issue comments and review feedback (e.g., "add some interaction between corridors", "transpose the animation", "remove uninformative plots").  No code was written manually.
- **LLM role**: The LLM autonomously designed the network topology, wrote all Python code (`generate_data.py`, `generate_visualizations.py`, `train_and_evaluate.py`), ran the simulations, trained the ML models, generated visualisations, wrote the technical report, and iteratively refined the experiment based on human feedback.
- **Tools used**: The LLM agent used [UXsim](https://github.com/toruseo/UXsim) for mesoscopic traffic simulation, TensorFlow/Keras for neural network training, scikit-learn for linear regression, and matplotlib for plotting.
- **Iterative refinement**: The experiment evolved through several rounds of human feedback — for example, the initial independent-corridor design was revised to include cross-corridor connections after the human pointed out that independent corridors make the prediction problem unrealistic.

This demonstrates how LLM coding agents can conduct end-to-end numerical experiments in specialised domains (traffic engineering, in this case) with only minimal human oversight providing strategic direction.
